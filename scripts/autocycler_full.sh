...
# Filter out reads shorter than 500 bp
filtered_reads=$(dirname "$reads")/filtered/$(basename "$reads")
mkdir -p "$(dirname "$filtered_reads")"
echo "Filtering out reads shorter than 500 bp..."
seqkit seq -m 500 "$reads" -o "$filtered_reads"
